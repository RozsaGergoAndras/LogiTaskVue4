...
// csak befejezett feladatok
const finishedTasks = computed(() =>
  tasks.value.filter(t => t.state2date)
);

// rendezett lista
const sortedTasks = computed(() =>
  [...finishedTasks.value].sort((a, b) => {
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(finishedTasks.value.length / pageSize.value))
);
...
function formatState(s) {
  return s === 2 ? 'Befejezett' : s === 1 ? 'Megkezdett' : 'Kiadott';
}
...
async function fetchAssigned() {
  ...
  errorMessage.value = '';
  try {
    const res = await fetch('http://127.0.0.1:8000/api/tasks/worker', {
      method: 'POST',
      headers: {
